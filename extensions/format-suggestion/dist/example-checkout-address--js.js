(() => {
  ...
  function fetchSuggestions(_field, _value, _selectedCountryCode, signal) {
    return __async(this, null, function* () {
      return fetch(
  ...
  // extensions/format-suggestion/src/format-suggestion.js
  var format_suggestion_default = extension(
    "purchase.address-autocomplete.format-suggestion",
    (_0) => __async(void 0, [_0], function* ({ target }) {
      const { selectedSuggestion } = target;
      const response = yield fetchSuggestions2();
      const { result } = yield response.json();
      const formattedAddress = result.suggestions.find(({ global_address_key }) => global_address_key === selectedSuggestion.id);
      return formattedAddress;
    })
  );
  function fetchSuggestions2() {
    return __async(this, null, function* () {
